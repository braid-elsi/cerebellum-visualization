...
function setup() {
    ...
    edges1 = generateLines(bgCircles, midCircles);
    edges2 = generateLines(midCircles, fgCircles);
}
...
function draw() {
    ...
    let bgOffsetX = scrollPosX * 0.3,
        bgOffsetY = scrollPosY * 0.3;
    let midOffsetX = scrollPosX * 0.6,
        midOffsetY = scrollPosY * 0.6;
    let fgOffsetX = scrollPosX * 1.2,
        fgOffsetY = scrollPosY * 1.2;
    ...
    // Draw connections:
    drawConnections(edges1, bgOffsetX, bgOffsetY, midOffsetX, midOffsetY, 100);
    drawConnections(edges2, midOffsetX, midOffsetY, fgOffsetX, fgOffsetY, 150);
}
...
function generateLines(layer1, layer2) {
    ...
    for (let c1 of layer1) {
        const filteredItems = getRandomItems(
            layer2,
            getRandomInt(1, layer2.length),
        );
